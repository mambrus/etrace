...
void opts_help(FILE *file, int flags)
{
    if (file && flags & HELP_USAGE) {
        fprintf(file, "%s",
                "Usage: etrace [-DzhutV]\n"
                "            [-T time] [--harvest-time=time]\n"
                "            [-P time] [--period-time=time]\n"
                "            [-c name] [--clock=name] \n"
                "            [-s size] [--buffer-size=sz_kb] \n"
    ...
    if (file && flags & HELP_LONG) {
        fprintf(file, "%s",
                ...
                "  -T, --harvest-time=time    Harvest-time ("
                xstr(DEF_HTIME) " uS).\n"
                "                             I.e. the total time gathering traces is occurring.\n"
                "  -P, --period-time=time     Period-time ("
                xstr(DEF_PTIME) " uS).\n"
                "                             Time breathing (i.e. sleeping) in-between working\n"
                "                             transferring buffer to output.\n"
                ...
                "  -s, --buffer-size=sz_kb    ftrace buffer size. ("
                xstr(DEF_FTRACE_BUFF_SIZE_KB) " kB)\n"
                "                             Set if option seen, else not touched\n"
